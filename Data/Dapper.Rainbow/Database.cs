...
namespace Dapper
{
    ...
    public abstract partial class Database<TDatabase> : IDisposable where TDatabase : Database<TDatabase>, new()
    {
        public partial class Table<T, TId>
        {
            ...
            /// <returns></returns>
            /// Modified by PY 04/03/215 : Custom Id column name
            public int Update(TId id, dynamic data, string idColumnName)
            {
                List<string> paramNames = GetParamNames((object)data);

                var builder = new StringBuilder();
                builder.Append("update ").Append(TableName).Append(" set ");
                builder.AppendLine(string.Join(",", paramNames.Where(n => n != idColumnName).Select(p => p + "= @" + p)));
                builder.Append("where "+idColumnName+" = @"+idColumnName);
