...
namespace NClassifier.Bayesian
{
	public interface ICategorizedWordsDataSource : IWordsDataSource
	{
        ...
        WordProbability GetWordProbability(Category category, string word);

        /// <summary>
		/// Gets the probabilities for a set of words for different categories.
		/// </summary>
		/// <param name="categories">The categories to check against.</param>
		/// <param name="words">The set of words to calculate the probability of.</param>
		/// <returns>The word probability if the word exists, null otherwise.</returns>
        IEnumerable<WordProbability> GetWordsProbabilities(List<Category> categories, List<string> words);
        /// <summary>
        ...
        void AddMatch(Category category, string word);
        ...
        void AddNonMatch(Category category, string word);
	}
}